...
- 2026-08-23 · open Rocuronium's own popover to screenshot the redesigned menu bar UI · `statusitem --press` on the app's own SwiftUI `MenuBarExtra(.window)` item returns AX error -25200 and no window appears (window count 0 → 0, honestly unverified) — yet the same mechanism verifiably opened Phosphene's MenuBarExtra popover on 2026-08-22, so something about pressing *this* process's own status item from inside itself (or this MenuBarExtra configuration) refuses; unresolved · Kiri's own click
