// Generated by Scripts/embed-skill.py from .claude/skills/rocuronium — edit the skill and
...
"""##),
        File(path: "reference/acting.md", contents: ##"""
# Acting
...
**`shortcut --app X --keys cmd+a [--resolve-only] [--confirm]`** — resolves the **menu item** bound to those keys and presses it. No keystroke is sent, which is why it works on Chromium; it also means a shortcut with no menu item cannot be delivered this way (use `key`). The reply names `menuItem` ("Edit ▸ Export"). Dependable on the frontmost app; in the background, AppKit apps never validate their menus, so the press returns success and does nothing, foretold by `menuItemReportedDisabled` in the reply.

**The four editing chords bypass the menu.** `cmd+a`/`cmd+c`/`cmd+x`/`cmd+v` route to a direct accessibility edit on the focused text element — select-all sets the selected range; copy/cut read the selection into `NSPasteboard`; paste writes the pasteboard over the selection — and each reads back and reports a verdict ("copied N characters", or `noEffect` when nothing is selected). This is because the menu path fails these two ways: the Edit menu is dead in the background, and `AXPress` on Edit▸Copy does not populate the clipboard even when live (measured). The edit works in the background, takes no cursor, and needs a focused text element (the reply says so if there is none — click in first).

**Which input verb, in order.** `type` = printable text (AX value write, works backgrounded). `shortcut cmd+c|x|v|a` = clipboard/select-all with evidence. Other `shortcut`/`menu` chords = a **menu item**, so activate the app first or the press no-ops in the background. `key` = a bare **named** key (escape/return/tab/arrows…) with modifiers, never a letter chord.
...
"""##),
        File(path: "reference/environment.md", contents: ##"""
# Environment facts that trip agents
...
"""##),
        File(path: "reference/isolation.md", contents: ##"""
...
"""##),
        File(path: "reference/presence-and-refusals.md", contents: ##"""
...
"""##),
        File(path: "reference/reply-contract.md", contents: ##"""
# The reply contract
...
"""##),
        File(path: "reference/targeting-and-observing.md", contents: ##"""
...
"""##),
        File(path: "reference/vision.md", contents: ##"""
